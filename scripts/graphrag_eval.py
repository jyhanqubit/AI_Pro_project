...
CASES = [
    ...
    (
        "가장 부족한 개별 대여소는?",
        set(),
        True,  # not in event context -> honest refusal
...
if __name__ == "__main__":
    ...
    print(
        "Midtown/Brooklyn/3rd-event/per-station questions have NO relevant event (gold = empty).\n"
    )
    labels = [
    ...
    for key, label in labels:
        ...
        print(
            f"   ungrounded id leaks   : {r['ungrounded_id_leaks']}  (product filter가 표면 차단)"
        )
        print()
    print(
        "검색이 없으면(raw) 이벤트 자체를 지어냅니다(환각 10/10). 검색+grounding은 '없는 것 지어내기'를 "
