...
from optimization.classical.problem import RebalancingPlan
    ...
    def step(self, action: int) -> tuple[int, float, bool]:
        ...
        if alpha > 0.0:
            ...
            for m in plan.moves:
                ...
                self.bikes[i] -= m.quantity
                self.bikes[j] += m.quantity
                moved += m.quantity

        # 2) realized demand hits (same order/semantics as mpc.simulate).
        short = over = 0.0
        for j in range(z):
            net = self._realized[self.t, j]
            if net < 0:
                demand_out = -net
                served = min(self.bikes[j], demand_out)
                short += demand_out - served
                self.bikes[j] -= served
            else:
                room = self.capacity[j] - self.bikes[j]
                accepted = min(room, net)
                over += net - accepted
                self.bikes[j] += accepted
