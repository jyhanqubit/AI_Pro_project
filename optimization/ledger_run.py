...
from optimization.ledger import account, oracle_stock
...
def _sensitivity(
    model: np.ndarray, base: np.ndarray, actual: np.ndarray, A: LedgerAssumptions
) -> list[dict[str, Any]]:
    ...
    for se in (A.shortage_externality * 0.5, A.shortage_externality, A.shortage_externality * 2.0):
        for op in (A.overflow_penalty * 0.5, A.overflow_penalty, A.overflow_penalty * 2.0):
            ...
            out.append(
                {
                    "shortage_externality": round(se, 4),
                    "overflow_penalty": round(op, 4),
...
def main(argv: list[str] | None = None) -> None:
    ...
    A = load_assumptions()
    print(
        f"assumption set: {A.version} (sourced={A.sourced})  oracle_upper_bound={A.oracle_is_upper_bound}"
    )
        ...
        per_window.append(
            {
                ...
                "model_net": round(mnet, 2),
                "no_action_net": round(bnet, 2),
                "oracle_net": round(onet, 2),
                "model_lift_net": round(mnet - bnet, 2),
    ...
    for name, led in report["by_policy"].items():
        print(
            f"  {name:14s} net={led['net']:>12.2f}  short_u={led['shortage_units']:>9.0f}  "
    ...
    print(
        f"sensitivity: model_minus_no_action_net sign across {len(sens)} cost settings = "
