"""Real LLM event-extraction provider (Anthropic / Claude). CLAUDE.md section 8.

Opt-in only — Demo Mode and every test use the deterministic ``MockLlmProvider`` and never this
one (``LLM_PROVIDER=mock`` by default). This provider implements the same ``LlmProvider`` interface:
it turns one article into raw candidate event dicts, which the extractor then validates with
Pydantic, grounds against the article text, gates on confidence, and deduplicates.

Guardrails honoured here (section 8, §22):

* **Structured output** via strict tool use — Claude must return a schema-valid ``record_events``
  call, so the shape is guaranteed and the extractor's Pydantic validation is a second gate.
* **Grounded evidence only** — every evidence span the model returns is kept only if it is an exact
  substring of the article title/text; an event with no grounded span is dropped (never an
  ungrounded/fabricated event).
...
from config.events import ANTHROPIC_PROMPT_VERSION
...
def _stable_event_id(article_id: str, event_type: str) -> str:
    raw = f"{article_id}|{event_type}|{ANTHROPIC_PROMPT_VERSION}"
    return "evt_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


class AnthropicProviderUnavailable(RuntimeError):
    """Raised when the Anthropic SDK is not installed or no credentials are configured."""


class AnthropicLlmProvider(LlmProvider):
    """Real Claude-backed extractor (opt-in). Same interface as the mock; never used in Demo."""
    ...
    def _get_client(self):
        ...
        settings = get_settings()
        # Prefer an explicit configured key; otherwise let the SDK resolve ANTHROPIC_API_KEY /
        # an `ant auth login` profile from the environment.
        self._client = (
            anthropic.Anthropic(api_key=settings.llm_api_key)
    ...
    def extract(self, article: ArticleRecord) -> list[dict]:
        ...
        tool_input: dict | None = next(
            (b.input for b in response.content if getattr(b, "type", None) == "tool_use"), None
        )
        if not tool_input:
            ...
            candidates.append(
                {
                    ...
                    "extraction_prompt_version": ANTHROPIC_PROMPT_VERSION,
                }
            )
        return candidates
