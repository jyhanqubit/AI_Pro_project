...
구현 상태, 측정된 결과, 정직성 경계, 데이터 수집 현황, 남은 갭, V2 후보 방향을 한 곳에 정리합니다.
...
차량 재배치·추천·인센티브 의사결정 지원** 시스템. 핵심 규율은 **정직성**: 모든 값이
...
- **제약(정직)**: 이 샌드박스의 **공유 IP가 GDELT rate-limit(429)에 걸려** 대량(수백 건) 수집은
...
> 반드시 §4·§7 말미의 정직성 불변을 유지하고, 각 단계에 **acceptance criteria + 측정 artifact +
