...
이벤트 피처 조사가 판정 불가로 끝난 뒤, **이론이 방향을 지정하는** 실험을 하나 수행했습니다.

품절은 과잉보다 2배 비싼데(OCS 가중치 2:1) 모델은 대칭 squared_error로 학습해 조건부 평균을
맞추고 있었습니다. 뉴스벤더 정리는 비용 최소 점 예측을 `q* = c_s/(c_s+c_o) = 0.667` 분위로
지정합니다. **실행 전에 이 예측을 artifact에 기록**하고(`newsvendor_q_star`), 손실함수만 바꿔
같은 rolling-origin 창에서 측정했습니다.
...
- **OCS 최솟값이 이론값 q=0.667과 일치**하고, 곡선이 그 지점을 중심으로 U자입니다.
- **3개 창 전부에서 q=0.667이 최적**입니다(−4.44% / −2.93% / −2.85%). 부호가 뒤집히는 창이 없습니다.
- 품절 대수: 25,184→18,704 / 27,327→19,854 / 27,262→20,105 (**평균 −26%**).
- 대가는 명시합니다 — **WAPE가 8.4% 나빠집니다.** 정확도와 운영 비용을 맞바꾼 것이므로 두 지표를
  항상 나란히 보고합니다.
- 중앙값(q=0.5)은 WAPE가 가장 좋지만 OCS는 기준보다 나쁩니다(+4.92%). 정확도 지표만 보면 운영상
  잘못된 모델을 고른다는 Phase 06의 발견을 재확인합니다.

재현: `make v2-quantile-cost` → `reports/v2/holdout/quantile_cost.json`

이 결과는 3장에 기록해 둔 한계("손실함수는 대칭이었다")를 실제로 메운 것입니다. 서빙 모델 자체는
아직 대칭 손실로 학습된 것을 씁니다 — 어떤 손실을 프로덕션에 둘지는 정확도와 운영 비용 중 무엇을
목표로 하느냐의 선택이고, 그 선택을 근거와 함께 제시하는 것이 이 실험의 역할입니다.

## 조건부 결론 (2026-08-19 최종) — LLM 뉴스 피처는 이벤트의 공간 해상도에 따라 갈립니다

`make v2-news-conditions` → `reports/v2/llm_value/news_feature_conditions.json`

단일 시드로는 판단할 수 없습니다. A1과 A2는 test 행의 약 6%에서만 입력이 다르므로 트리 학습의 난수가
비교를 지배하고, 같은 창이 시드에 따라 +2.23과 −2.26 사이를 오갑니다. 시드 10개의 예측을 평균한
앙상블로 그 노이즈를 제거하면 다음이 나옵니다.

| 창 | 이벤트당 평균 borough | 전역(4+) 비율 | 앙상블 gain | CI95 | 판정 |
|---|---|---|---|---|---|
| 2026-02 | 4.2 | 80% | −0.96 | [−1.53, −0.42] | 악화 |
| 2026-03 | 3.7 | 67% | −0.86 | [−1.40, −0.28] | 악화 |
| 2026-05 | 2.3 | 29% | −0.76 | [−1.42, −0.23] | 악화 |
| 2026-06 | **2.0** | **25%** | **+1.24** | **[0.83, 1.64]** | **개선** |

이벤트 확산도로 정렬하면 gain이 **단조 상승**하고 약 2개 borough에서 부호가 바뀝니다.

**메커니즘.** 전역 이벤트(폭설 등)는 5개 borough에서 동시에 켜지므로 borough를 구분하는 정보량이 0이고,
"그날은 특별한 날"이라는 시간 정보는 이미 calendar 피처가 갖고 있습니다. 남는 것은 노이즈뿐입니다.
반대로 지역 특정 이벤트(집회·공연)는 어느 구역이 언제 오르는지를 지목해 실제 공간 정보를 더합니다.

**기각한 대안 설명.** "초기 창은 학습 데이터가 부족해서 음수였다"는 가설은 테스트셋을 고정하고 학습
기간만 바꿔 검증했고 기각됐습니다. 6월은 1개월 −0.32 → 5개월 +1.24로 오르지만, 5월은 1개월 0.00 →
4개월 −0.76으로 **반대 방향**입니다. 학습량이 원인이라면 두 창이 같은 방향으로 움직여야 합니다.

**한계.** 정보 창이 4개뿐이라 단조 관계는 강하게 시사되지만 확립된 것은 아닙니다. 메커니즘의 직접
검증(4개 이상 borough 이벤트 제거)은 23건 중 12건이 빠져 커버리지 임계 아래로 떨어져 확인도 반증도
하지 못했습니다. 학습 5개월은 계절성과 이벤트 효과를 분리하기에 짧고, 1년치가 필요합니다.

## 정정 (2026-08-19) — borough 패널의 데이터 결함과 그 여파

이 절은 하루 동안 두 단계로 진행된 조사의 **최종 상태**입니다. 중간 단계에서 내렸던
`sign_flips` / `consistently_negative` 판정은 **결함 있는 패널에서 나온 것이라 폐기**했으며, 아래가
현재 유효한 결론입니다.

### 결함 두 가지

1. **Jersey City 트립 혼입** — borough 배정이 nearest-centroid인데 도시 경계 검사가 없어, NYC 아카이브와
   함께 놓인 JC/Hoboken 트립이 NYC borough로 흡수됐습니다. 특히 서쪽에 있는 **Staten Island**로 대량
   유입돼 2026-06 SI 시간 셀이 **1개 → 486개**로 부풀었습니다(`borough_event_lift`의 `+1.65%`가 여기서
   나옴).
2. **NYC→뉴저지 도착의 오집계** — arrivals는 도착지 좌표로 배정되므로, JC 파일을 넣지 않아도 맨해튼에서
   저지시티로 건너간 트립이 **"맨해튼 도착"으로 기록**되어 `arr_*` 피처를 부풀렸습니다. 이 결함은 모든
   borough 실험에 있었습니다.

수정: `borough_of()`에 허드슨강 서쪽 거부 규칙(경계값은 실측 — NYC 최북단 서쪽 좌표 lat 40.6851,
JC 최남단 40.6922, 그 사이 780 m 간격)과 15 km 거리 상한을 넣고, 거부 건수를 사유별로 보고합니다
(§6.1). 회귀 테스트 7개: `tests/unit/test_borough_boundary.py`.

### 수정 후 rolling-origin 재측정 (현재 유효)

`make v2-llm-value-rolling` → `reports/v2/llm_value/rolling_origin_ablation.json`

| 창 | permit 행 | A1−A0 (permit 이벤트) | A2−A1 (LLM 뉴스) |
|---|---|---|---|
| 2026-05 | 2,600 | +0.42 CI [−2.00, 2.71] `inconclusive` | −0.58 CI [−1.72, 0.59] `inconclusive` |
| 2026-06 | 2,612 | +1.91 CI [−0.24, 3.92] `inconclusive` | **+2.23 CI [1.10, 3.35]** `measured_improvement` |
| 2026-07 | 56 | `blocked_data` | `blocked_data` (news 0행) |

- **A1−A0**: 두 창 모두 점추정은 양수지만 CI가 0을 포함 → **판정 불가**. 개선도 악화도 주장하지 않습니다.
- **A2−A1**: 6월에 **유의한 개선**, 5월은 판정 불가 → 측정 가능한 2창 중 1창에서 유의.
  이전의 "모든 창에서 음수" 결론은 결함 패널의 산물이었습니다.
- 중간에 보고했던 `+3.96`(5월) / `−3.46`(6월)은 결함 패널 값이며 현재는 각각 `+0.42` / `+1.91`입니다.

### 6월 이상치의 원인 가설 — 검증에서 지지받지 못했습니다

6창 중 2026-06만 A2−A1이 유의하게 양수였습니다(+2.23). 원인을 찾다가, 창을 이벤트의 평균 borough
수로 정렬하면 gain 순서와 정확히 일치한다는 것을 발견했습니다(Spearman ρ = −1.00, n=4).

| 창 | 평균 borough | citywide 비율 | 주요 유형 | A2−A1 |
|---|---|---|---|---|
| 2026-06 | 2.0 | 25% | Public Gathering / Large Venue | +2.23 |
| 2026-05 | 2.3 | 29% | Large Venue 중심 | −0.58 |
| 2026-03 | 3.7 | 67% | Weather Shock 중심 | −0.86 |
| 2026-02 | 4.2 | 80% | Weather Shock 중심 | −0.96 |

가설: 전역(citywide) 이벤트는 5개 borough에서 동시에 켜져 공간 정보가 0이므로 calendar 피처와
중복되고 노이즈만 더한다. 이는 V2-03에 사전 등록된 **precise-location** 조건과 부합합니다.

**검증**(`--max-event-boroughs 3`, `reports/v2/llm_value/rolling_origin_localized.json`): 4개 이상
borough에 걸친 이벤트 12/23건을 제외하고 같은 6창을 재측정했습니다. 결과는 **가설을 지지하지
않습니다.**

- 뉴스 활성 행이 약 57% 줄어 대부분의 창이 커버리지 임계(100행) 아래로 떨어졌습니다. 검정력이
  부족해 결정적 반증도 되지 못합니다.
- 읽을 수 있는 유일한 신호가 반대 방향입니다. 6월은 이미 대부분 지역 특정 이벤트였으므로 가설대로면
  개선이 유지·확대되어야 하는데, **+2.23 → +0.374로 축소**됐습니다. 즉 6월의 개선에는 제거한 전역
  이벤트도 기여하고 있었습니다.

**결론:** 6월 이상치의 원인은 규명하지 못했습니다. n=4에서 ρ=−1.00이 나올 확률은 약 4%로, 우연으로도
충분히 설명됩니다. 그럴듯한 메커니즘 서사가 붙는다고 해서 근거가 되지는 않는다는 사례로 남깁니다.

### 원래 주장들의 현재 상태

| 주장 | 원래 근거 | 현재 |
|---|---|---|
| A1−A0 구조화 이벤트 lift (+2.69%) | 2026-05 단일 분할 | 깨끗한 패널에서 +0.42, **판정 불가** |
| v1 이벤트 lift (WAPE −1.65%) | 2026-06 단일 홀드아웃 (JC 혼입) | 수정 후 +1.03%, CI [−0.24, 3.90] **판정 불가** |
| LLM-from-news 증분이 음수 | 결함 패널 | **철회** — 6월은 유의한 개선 |

즉 이벤트 피처에 대해 현재 저장소가 주장하는 것은 **"이 데이터로는 판정 불가"**이며, 개선 주장도
악화 주장도 하지 않습니다. 승격 모델이 seasonal naive를 세 창 모두에서 이긴다는 결과는 H3 격자
경로라 이 결함과 **무관하게 유지**됩니다.

- 기존 artifact는 삭제하지 않았고, 각 단계의 수치를 커밋 메시지와 이 문서에 남겼습니다.
- density curve와 quality ablation은 2026-05 단일 창 기반이라 같은 조건부입니다(패널 자체는 깨끗함).

### 부록: v1 `-1.65%` 실험의 추적 기록

위 결과를 확인하는 과정에서 v1 실험(`borough_event_lift`, 2026-06 홀드아웃)도 현재 데이터로 다시
돌렸습니다. 같은 명령, 같은 test 구간인데 결과가 반대로 나옵니다.
...
| 상대 변화 | **+1.65% 개선** (JC 혼입) | **-1.94%** (NYC만, 가드 전) |
| 95% CI | [0.36, 5.11] | **[-6.09, -0.86]** |

- 재현 결과의 행 수(13,630 / 2,881 / 이벤트 2,612)는 위 rolling-origin 검증의 6월 창과 **정확히
  일치**하고 mean_gain도 -3.42 vs -3.46으로 같습니다. 두 검증이 서로를 뒷받침합니다.

**원인 — Jersey City 트립의 borough 오배정:** 행 수 차이를 추적한 결과, 원본은 NYC 아카이브와
**Jersey City 아카이브를 같은 디렉터리에 넣고** 실행한 것으로 확인됩니다. `stream_borough_cells`는
가장 가까운 borough 중심점으로 배정하는데, Jersey City(약 40.72, -74.04)에서 가장 가까운 NYC
중심점이 유일하게 서쪽에 있는 **Staten Island(40.579, -74.15)**입니다.
...
이 486이 test 행 차이(3,366 - 2,881 = 485)와 일치합니다. 원본 test set의 약 14%가 **뉴저지 수요에
"Staten Island" 라벨이 붙은 행**이었고, 거기에 **실제 Staten Island의 NYC permit 이벤트**가 결합돼
있었습니다. 서로 무관한 두 데이터가 같은 행에 묶인 것이므로, `+1.65%`는 이벤트 신호가 아니라
**데이터 오배정의 산물**로 보아야 합니다.

- `reports/borough_event_lift.json`은 **NYC만 사용한 재현 결과로 갱신**했습니다. 원본 수치는 이 표와
  `docs/EVENT_LIFT_FINDINGS.md` 상단 정정 블록에 보존했습니다.
- **재발 방지:** borough 배정은 nearest-centroid라 입력 좌표가 NYC 밖이어도 조용히 배정됩니다.
  경계 밖 좌표를 거부하거나 거리 상한을 두는 검사가 필요하고, artifact에 입력 파일 목록을 기록해야
  합니다(§7.1 provenance).

### 부록 종합 (위 '현재 상태' 표가 최종입니다)

| 주장 | 원래 근거 | 현재 상태 |
|---|---|---|
| A1-A0 구조화 이벤트 lift (+2.69%) | 단일 분할 | 가드 후 +0.42, 판정 불가 |
| v1 이벤트 lift (-1.65% WAPE) | 2026-06 단일 홀드아웃 | 현재 데이터로 -1.94% 악화 → 철회 |
| LLM-from-news 증분 | 결함 패널 | 가드 후 6월 유의한 개선 → 부정 결론 **철회** |
| 날씨 피처 | — | CI [-0.0103, -0.0033] 음수 → **유지** (부정 결론) |

이벤트 피처에 대한 현재 결론은 위 '수정 후 rolling-origin 재측정' 절을 따릅니다.
