...
### 이어지는 확인: v1의 `-1.65%` 이벤트 lift도 재현되지 않습니다
...
### 종합: 이벤트 피처의 긍정적 주장은 현재 모두 철회 상태입니다

| 주장 | 원래 근거 | 현재 상태 |
|---|---|---|
| A1-A0 구조화 이벤트 lift (+2.69%) | 단일 분할 | rolling origin에서 `sign_flips` → 철회 |
| v1 이벤트 lift (-1.65% WAPE) | 2026-06 단일 홀드아웃 | 현재 데이터로 -1.94% 악화 → 철회 |
| LLM-from-news 증분 | — | 측정 가능한 모든 창에서 음수 → **유지** (부정 결론) |
| 날씨 피처 | — | CI [-0.0103, -0.0033] 음수 → **유지** (부정 결론) |

즉 이 저장소에 남은 이벤트 관련 결론은 **부정적 결과와 그 원인 규명**(밀도·시각 정밀도·위치
정밀도·선행성)이며, 예측 개선 주장은 없습니다. 승격 모델이 seasonal naive를 세 창 모두에서 이긴다는
결과는 이와 무관하게 유지됩니다.
