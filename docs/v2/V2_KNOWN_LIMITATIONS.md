...
- **단일 시드 비교는 신뢰할 수 없다 (2026-08-19):** A1 과 A2 는 test 행의 약 6% 에서만 입력이 다르므로,
  나머지 94% 에서 두 arm 의 예측 차이는 오직 트리 학습의 난수에서 나온다. 같은 창이 `random_state` 만
  바꿔 +2.23 과 −2.26 사이를 오간다(둘 다 CI 가 0 을 제외). 이 계열의 비교는 **반드시 시드 앙상블**로
  측정해야 하며, 단일 시드 수치는 어떤 방향으로도 인용하지 않는다.
- **조건부 결과의 표본 한계:** 뉴스 피처 기여가 이벤트 공간 해상도에 단조하다는 관계는 **정보 창 4 개**에
  기반한다. 강하게 시사되지만 확립된 것은 아니다. 메커니즘의 직접 검증(4 개 이상 borough 이벤트 제거)은
  23 건 중 12 건이 빠져 활성 행이 57% 감소, 커버리지 임계 아래로 떨어져 확인도 반증도 하지 못했다.
- **학습 5 개월은 계절성 학습에 부족:** 계절 효과와 이벤트 효과를 분리하려면 최소 1 년치가 필요하다.
  현재 결과는 그 분리가 안 된 상태의 측정이다.
