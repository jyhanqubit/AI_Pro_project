...
| Copilot correctness + relevance | V2-06 | `reports/v2/copilot/{correctness,graphrag}_benchmark.json` | **offline_benchmark** | **Typed-tool**: 20 Q, real claude routing 1.0/1.0/1.0, halluc=0 (keyword 3, FAIL). **GraphRAG @ scale** (real graph 2,895 events): vs a fair flat-retrieval baseline (6/21, refuses 6/6, 0 halluc) GraphRAG is 21/21 — but task is graph-structural so graph is high BY CONSTRUCTION (not a fair bakeoff; caveats recorded) |
