...
| LLM-from-news incremental value | V2-03 | `reports/v2/llm_value/incremental_value_borough.json` (+`incremental_value.json`) | **measured (negative)** | Fair test (test window has 216 news rows): A2−A1 WAPE 0.1047→0.1075, CI [−6.02,−3.71], **net LLM value −$23,730**. LLM-from-news is net-negative; H3/JC run also null |
