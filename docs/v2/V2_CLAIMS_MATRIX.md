...
모든 V2 API 응답과 UI 지표는 아티팩트로 추적 가능해야 하며 정직하게 라벨링되어야 한다. 이
...
| 구조화된 event feed lift (permitted) | V2-03 | `reports/v2/llm_value/incremental_value_borough.json` | **measured, 재현 실패 (2026-08-19 정정)** | Borough/NYC trips, test May: A1−A0 WAPE 0.1069→0.1047, CI [1.87,5.88], +$33k. **이 결과는 2026-05 단일 창에서만 성립한다.** rolling origin 재검증에서 2026-06은 −3.46 CI [−6.10,−0.92]로 부호가 뒤집혀(`sign_flips`) **개선 주장을 철회**한다 — 아래 행 참조 |
| **LLM 뉴스 피처의 조건부 기여** | V2-03 | `reports/v2/llm_value/news_feature_conditions.json` | **measured (조건부)** | 시드 10 앙상블로 난수를 제거하면 gain 이 이벤트의 공간 해상도에 **단조**한다: 평균 borough 4.2 개 −0.96 / 3.7 개 −0.86 / 2.3 개 −0.76 / **2.0 개 +1.24 (CI [0.83, 1.64])**. 약 2 개 borough 에서 부호 전환. 메커니즘 = 전역 이벤트는 모든 borough 를 동시에 켜 공간 정보량이 0 이 되고 calendar 와 중복. 학습량 가설은 테스트셋 고정 대조로 기각. 한계: 정보 창 4 개, 직접 검증은 커버리지 미달 |
| **Rolling-origin 재현성 검증 (A0/A1/A2)** | V2-03 | `reports/v2/llm_value/rolling_origin_ablation.json` | **measured** | 월별 창마다 세 arm 을 재학습: A1−A0 `sign_flips` (05 +3.96 / 06 −3.46 / 07 `blocked_data`), A2−A1 `consistently_negative` (측정 가능한 2 창 모두 음수). 단일 분할 CI 가 담지 못하는 학습 변동을 추가로 드러냄. 부정적 결론은 유지, 긍정적 결론은 철회 |
...
기계 검증 gate (envelope honesty, completion-artifact coverage, traceability) 와 함께. 현재
