...
| **LLM Feature Value metric** (did LLM features improve accuracy?) | V2-03 | `reports/v2/llm_value/incremental_value_borough.json#llm_feature_value_metric` | **measured** | Decision-grade metric = relative WAPE reduction on the **LLM-active subset** + block-bootstrap CI. Result: **`MEANINGFUL_NEGATIVE`**, active skill **−5.52%**, CI [−17.51, −0.98], n=336 → LLM features **measurably degrade** accuracy where they fire. Pre-declared thresholds (1% effect + CI≠0); pure fn, 6 unit tests |
