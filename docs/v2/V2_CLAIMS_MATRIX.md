...
| Signed LLM demand signal (direction from LLM) | V2-03 | `reports/v2/llm_value/signed_demand_contribution.json` | **measured (neutral, understood)** | LLM `demand_effect∈[−1,+1]` (blizzard −0.9, festival +0.5, LIRR shutdown +0.6). **LLM sign-correctness 0.77** (direction is right!), harm removed (−7.82%→−2.04%), but **`NO_MEANINGFUL_EFFECT`** CI [−7.26,+1.44]. Reason: autoregressive lags already encode ongoing-event demand → news signal **redundant**. Completes the why: sparsity + sign + redundancy |
| Post-processing correction (per-mechanism factors) | V2-03 | `reports/v2/llm_value/postprocess_contribution.json` | **measured (negative)** | Correct model output `pred + Σα_ch·signal_ch`, per-mechanism α calibrated on train. **`MEANINGFUL_NEGATIVE` −21.36%** CI [−102.6,−4.95] (WAPE 0.0883→0.0899). Fitted factors absurd/wrong-signed (α[gather]=−470, α[transit]=−478) → sparse calibration **overfits**, fails on test. Sparsity moves from model to post-proc; still can't pin a stable factor from ~19 events |
| Forecast-horizon sweep (event value vs lead time) | V2-03 | `reports/v2/llm_value/horizon_contribution.json` | **measured** | Tested whether losing recent lags at longer horizons lets events matter more. Permit A1−A0 is `MEANINGFUL_POSITIVE` **+2.69%** only at nowcast (h=1); neutral at h≥6 (baseline WAPE doubles → noisier). News neutral-to-negative at every horizon. The null is **not** a nowcasting artifact |
| **Root-cause: density curve + quality ablation** | V2-03 | `reports/v2/llm_value/{density_curve,quality_ablation}.json` | **measured** | Proves it's NOT just sparsity. Density curve: permit value dead at news-scale (≤100 events; news~19), appears ≥300 but non-monotonic → density necessary, not sufficient. Quality ablation (density held FULL): degrading exact-time (−0.33%), exact-borough (+0.53%), or forward-timing (+1.01%) EACH collapses the +2.69%. Cause = conjunction of dense+precise-time+precise-location+forward-looking; news fails all four; "more data" fixes only density |
| News condition audit (can news satisfy the 4?) | V2-03 | `reports/v2/llm_value/news_condition_audit.json` | **measured** | Only 2/23 news events forward-looking, 1/23 BOTH forward+precise (June, out of test). The satisfying subset is ~empty because news is coincident/retrospective by nature — a 4-D encoding can't add info the source lacks |
| Synthetic ceiling — post-correction capability | V2-03 | `reports/v2/llm_value/synthetic_ceiling.json` | **simulated** | Disclosed injection of forward-looking precise dense event shocks into real demand. LLM post-correction **+10.43% MEANINGFUL_POS** (feature +20.86%); news-scale density → INSUFFICIENT. Proves the method/post-correction CAN exploit good events (real-news null is a SOURCE problem). NOT a real-news claim |
| LLM enrichment of real permit feed (path A) | V2-03 | `reports/v2/llm_value/{permit_enrich,permit_typed}_contribution.json` | **measured (negative)** | On the real forward-looking permit feed (all 4 conditions met), LLM semantic structuring beyond the crude count did NOT help: imposed demand-direction **−3.39%**, factual type-buckets **−1.9%** (both vs crude count MEANINGFUL_NEGATIVE). Aggregate count is the ceiling at borough grain; finer structure overfits / needs per-H3 grain (blocked: permits borough-tagged). LLM demand value on this data = none beyond a count |
...
| All UI metrics resolve to artifacts | V2-07 | Cockpit → `services/api/v2_metrics.py` → `reports/v2/**` | **offline_benchmark / measured** (per metric) | Operator cockpit renders 7 metrics, each wrapped in a `ResultEnvelope` read from its artifact (no hard-coded numbers); rider preview + trip planner numbers come from the typed plan (faithfulness 1.0). Screenshot `docs/screenshots/v2_cockpit.png` |
| RL rebalancing (research-only) | RESEARCH | `reports/v2/research/rl_rebalancing.json` | **research** | Tabular Q-learning + PPO on the V2-04 simulator/ledger. Regret vs Oracle: PPO **202.9** < tabular **247.8**, both trail MPC **21.6**. `beats_mpc=false`; **no RL advantage claimed**. Blocked from all product surfaces by `ResultEnvelope`. Not a completion condition. See `docs/v2/V2_RESEARCH_RL.md` |

The V2-09 final audit (`make v2-final`, `scripts/v2_final_audit.py`) fills this matrix from
committed artifacts and mirrors it into `reports/v2/final/claim_matrix.json` — with three
machine-checked gates (envelope honesty, completion-artifact coverage, traceability). Current
verdict: **PASS — V2_COMPLETE** (31 artifacts; see `reports/v2/final/v2_final_audit.md`). Algorithm
principles + metric definitions: `docs/v2/V2_ALGORITHMS.md`.
