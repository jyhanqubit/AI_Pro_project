...
| No-Event vs Rule-Event vs LLM-Event lift | V2-03 | `reports/v2/llm_value/incremental_value.json` (+`_borough.json`) | **blocked_data** (`insufficient_event_overlap`) | H3/JC: ΔWAPE=0 CI[0,0]. Borough/NYC (13.9M trips): A2−A1 insufficient (4/371 news borough-attributed, 0 in test), net LLM value −$13,814. LLM-from-news unmeasurable at **both** grains — bottleneck is news sparsity, not geography |
