...
def run(
    data_dir, events_path, news_path, raw_events, perm_events, test_from, target=PRIMARY_TARGET
):
    ...
    perm_vs_a1 = llm_feature_value(
        y_test, preds["A1_plus_permitted"], preds["A2_news_perm"], perm_active, blocks
    )
    perm_vs_raw = llm_feature_value(
        y_test, preds["A2_news_raw"], preds["A2_news_perm"], perm_active, blocks
    )
    ...
    return {
        ...
        "raw_news_diag": raw_diag,
        "permitized_diag": perm_diag,
        "test_rows_raw_active": int(raw_active.sum()),
        "test_rows_perm_active": int(perm_active.sum()),
        "arms": arms,
...
def main(argv=None) -> int:
    ...
    (OUT_DIR / "permitize_contribution.json").write_text(
    ...
    print(
        f"permitized vs A1      : {pa['decision']}  skill={pa['llm_active_skill_pct']}%  CI={pa['active_error_gain_ci95']} (n={pa['n_llm_active_rows']})"
    )
    print(
        f"permitized vs raw-news: {pr['decision']}  skill={pr['llm_active_skill_pct']}%  CI={pr['active_error_gain_ci95']}"
    )
