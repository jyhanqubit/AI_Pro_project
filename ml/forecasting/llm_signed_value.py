...
def run(data_dir, events_path, news_path, signed_events, test_from, target=PRIMARY_TARGET):
    ...
    sig_idx, sig_diag = build_signed_demand_index(
    ...
    for r in rows:
        ...
        if rec["news_demand_signal"] != 0.0:
            examples.append(
                (r.zone_id, hk, round(rec["news_demand_signal"], 3), float(r.targets[target]))
            )
    df = (
    ...
    lfv = llm_feature_value(
        y_test, preds["A1_plus_permitted"], preds["A_plus_signed_demand"], active, blocks
    )
    ...
    if lag is not None:
        ...
        sign_hits = {
            "n": int(m.sum()),
            "sign_agreement": round(float(agree.mean()), 3) if m.sum() else None,
        }
    ...
    return {
        ...
        "signed_diag": sig_diag,
        "test_rows_signal_active": int(active.sum()),
        "example_signed_values": [
            {"borough": b, "hour": h, "news_demand_signal": s, "actual_departures": d}
...
def main(argv=None) -> int:
    ...
    (OUT_DIR / "signed_demand_contribution.json").write_text(
        json.dumps(res, indent=2), encoding="utf-8"
    )

    print(
        f"train={res['n_train_rows']} test={res['n_test_rows']} signal_active={res['test_rows_signal_active']} "
    ...
    for e in res["example_signed_values"]:
        print(
            f"  {e['borough'] + ' ' + e['hour']:26s} signal={e['news_demand_signal']:+.3f}  actual_dep={e['actual_departures']:.0f}"
        )
    ...
    print(
        f"SIGNED demand value vs A1: {lfv['decision']}  skill={lfv['llm_active_skill_pct']}%  "
